...
static int get_key_value(struct mytp_event *event, struct mytp_data *data, int i);
static int mytp_input_dev_report_key_event(struct mytp_event *event, struct mytp_data *data);
static int mytp_input_dev_report_b(struct mytp_event *event, struct mytp_data *data);
...
static irqreturn_t mytp_interrupt(int irq, void *dev_id)
{
	...
	if (mytp_input_dev_report_key_event(&data->event, data))
	{
		mytp_input_dev_report_b(&data->event, data);
	}
				...
				for (i = 0; i < data->pdata->key_number; i++)
				{
					...
					if(key_value)
					{
								...
								event->au8_touch_event[i] == 2)
						{
							...
							PRINT_INFO("Key%d(%d, %d) DOWN!", key_value - 1, event->au16_x[i], event->au16_y[i]);
						}
						else
						{
							key_flag = 1;
							input_report_key(data->input_dev, data->pdata->keys[key_value - 1], 0);
							PRINT_INFO("Key%d(%d, %d) Up!", key_value - 1, event->au16_x[i], event->au16_y[i]);
						}
					}
				}
			}
...
static int mytp_input_dev_report_b(struct mytp_event *event, struct mytp_data *data)
{
	...
	for (i = 0; i < event->touch_point; i++)
	{
		...
		if (event->au8_touch_event[i] == FTS_TOUCH_DOWN || event->au8_touch_event[i] == FTS_TOUCH_CONTACT)
		{
			...
			if (event->area[i] <= 0)
			{
				PRINT_INFO("[B]Illegal touch-major: %d", event->area[i]);
				event->area[i] = 1;
			}
			...
			PRINT_INFO("[B]P%d(%d, %d)[tm:%d] DOWN!", event->au8_finger_id[i], event->au16_x[i], event->au16_y[i], event->area[i]);
		}
		else
		{
			uppoint++;
			...
			data->touchs &= ~BIT(event->au8_finger_id[i]);
			PRINT_INFO("[B]P%d UP!", event->au8_finger_id[i]);
		}
	}
