...
all_data_transfer_cmd="All Data Transfer"
    ...
    if ser.isOpen():
        ...
        while test_count < TEST_COUNT_MAX:
            ...
            if checksum == int(ord(imu_data[-1])):
                ...
                continue
            #time.sleep(0.05)
            systimestamp=int(ord(imu_data[12])*0x1000000+ord(imu_data[13])*0x10000+ord(imu_data[14])*0x100+ord(imu_data[15]))
            systimestamp=systimestamp/2
            imu_view="%6d," %(test_count)
            imu_view+="%8d," %(systimestamp)
            temp=int(ord(imu_data[16]))
            if temp > 0x7f :
            ...
            if False:
                ...
                gyro_x = hex_to_short_int(imu_data[0],imu_data[1],False)
                gyro_y = hex_to_short_int(imu_data[2],imu_data[3],False)
                gyro_z = hex_to_short_int(imu_data[4],imu_data[5],False)
                acc_x = hex_to_short_int(imu_data[6],imu_data[7],True)
                acc_y = hex_to_short_int(imu_data[8],imu_data[9],True)
                acc_z = hex_to_short_int(imu_data[10],imu_data[11],True)
