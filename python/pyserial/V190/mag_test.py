...
while count < MAX_COUNT:
    ...
    if ser.isOpen():
        ...
        while test_count < TEST_COUNT_MAX:
            mag_data = ser.read(9)
            crc = 0  ^ ord(mag_data[2]) ^ ord(mag_data[3]) ^ ord(mag_data[4]) ^ ord(mag_data[5]) ^ ord(mag_data[6]) ^ ord(mag_data[7])
            if crc != ord(mag_data[8]) :
                print "[CRC Failed] x:%02x:%02x,y:%02x:%02x,z:%02x:%02x,crc:%02x:%02x"%ord(mag_data[2]),ord(mag_data[3]),ord(mag_data[4]),ord(mag_data[5]),ord(mag_data[6]),ord(mag_data[7]),(ord(mag_data[8]),crc)
                continue
            if ord(mag_data[2]) != 0 :
            ...
            mag_view = "x:%+10.8f,y:%+10.8f,z:%+10.8f"%(mag_data_x*0.0015,mag_data_y*0.0015,mag_data_z*0.0015)
